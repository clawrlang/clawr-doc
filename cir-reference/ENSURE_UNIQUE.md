...
# [Clawr Intermediate Representation](./README.md) : Statements : `ENSURE_UNIQUE`
...
- The operation `MUST` be injected before assigning to the fields of an `ISOLATED` referenced-counted variable or field.
- The operation `MUST` be injected before calling a `mutating:` method on an `ISOLATED` referenced-counted variable or field.
