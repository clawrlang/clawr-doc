...
<style type="text/css" href="../admonitions.css"></style>
...
The output here is `[1 + 4, 1 + 5, 1 + 6, ..., 1 + 18]` or `[5, 6, 7, ..., 19]`. The runtime will iterate all through `y <- ys` for each element in `x <- xs`, but — since `ys` is infinite in size — we can never finish the `ys` for `x == 1`, and we will never see the 2.
...
- `y'` refers to the value of `y` in the previous iteration (undefined in the first)
...
| `{>`                 | temporal generator     |
| <code>&#124;></code> | state introduction     |

---

> [!question] AI Prompt
>
> Could the syntax be used to split a list in pairs? Like [1, 2, 3, 4] -> [(1, 2), (3, 4)]. (I suppose, if there was a fifth element it would have to be ignored unless there was also a sixth to pair it with.)
>
...
function formatAdmonitions() {
  const calloutPs = [...document.querySelectorAll('blockquote')].map(bq => bq.querySelector('p')).filter(p => /^\[!(.*)\]/.test(p.textContent))
